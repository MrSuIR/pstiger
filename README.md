...
# Запуск бенчмарка интерпретатора
dotnet run -c Release --project tests/Interpreter.Benchmarks

# Запуск бенчмарка компилятора
dotnet run -c Release --project tests/Compiler.Benchmarks
```
...
.NET SDK 10.0.104
  ...
  Job-HDDYAJ : .NET 10.0.4 (10.0.4, 10.0.426.12010), X64 RyuJIT x86-64-v3
```

### Бенчмарк интерпретатора без VM
...
### Бенчмарк интерпретатора с VM

Ветка `11_virtual_machine`, коммит `f3cde14214dcbce53f0439595ce24f20708e1646`, интерпретатор использует виртуальную машину:

| Method         | N     |      Mean |     Error |    StdDev |
|----------------|-------|----------:|----------:|----------:|
| ListPrimesUpTo | 1000  |  2.320 ms | 0.0540 ms | 0.0321 ms |
| ListPrimesUpTo | 10000 | 20.450 ms | 0.2538 ms | 0.1510 ms |
| ListPrimesUpTo | 50000 | 65.683 ms | 4.1960 ms | 2.7754 ms |

Заметно двухкратное ускорение от перехода на виртуальную машину, несмотря на полное отсутствие оптимизаций.

## Бенчмарк скомпилированной программы

Ветка `25_records`, коммит `9461302168fae7117717cae262520492e8e7f99e`

- компилятор генерирует MSIL и исполняемый файл .NET
- затем бенчмарк запускает исполняемый файл .NET в отдельном процессе

| Method         | N     |     Mean |    Error |   StdDev |
|----------------|-------|---------:|---------:|---------:|
| ListPrimesUpTo | 1000  | 45.13 ms | 2.207 ms | 1.460 ms |
| ListPrimesUpTo | 10000 | 48.64 ms | 2.650 ms | 1.753 ms |
| ListPrimesUpTo | 50000 | 60.11 ms | 1.834 ms | 1.213 ms |

В результатах видно следующее:

- накладные расходы на запуск процесса составляют порядка 45 миллисекунд
- несмотря на это, при N = 50 000 скомпилированная программа всё равно обгоняет интерпретатор с виртуальной машиной
- если же вычесть накладные расходы, то скомпилированная в MSIL/.NET программа примерно в 4 раза быстрее интерпретатора

### Бенчмарк алгоритма на C#
