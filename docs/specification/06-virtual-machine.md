...
8. `StoreArray` сохраняет в массив `EVAL[^2]` по индексу `EVAL[^1]` значение `EVAL[^3]`
...
11. `StoreField [Name]` сохраняет в поле `[Name]` структуры `EVAL[^1]` значение `EVAL[^2]`
...
27. `JumpIfFalse [Target]` выполняет условный переход, если условие ложно
    ...
    - Снимает одно значение из стека
28. `CallBuiltin [Code]` вызывает встроенную функцию
