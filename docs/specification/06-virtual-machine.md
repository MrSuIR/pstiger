...
4. `StoreVar [Name]` записывает в переменную значение `EVAL[^1]`
    - Операнд хранит имя переменной
    - Убирает значение со стека вычислений
5. `CreateArray` создаёт массив размера `EVAL[^2]`, инициализируя каждый элемент значением `EVAL[^1]`
    - Убирает два значения со стека вычислений, помещает результат в стек вычислений
6. `StoreArray` сохраняет в массив `EVAL[^3]` по индексу `EVAL[^2]` значение `EVAL[^1]`
    - Убирает три значения со стека вычислений
7. `LoadArray` читает элемент массива `EVAL[^2]` по индексу `EVAL[^1]`
...
9. `LoadField [Name]` читает поле структуры `EVAL[^1]` с именем `[Name]`
